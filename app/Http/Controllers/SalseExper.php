...
//امتیازات این ماه بعد از تخلیه
        ...
        //محاسبه امتیازات اضافی بازاریابها
        $all_monthly_bonuses=0;
        $historyExist=DB::select("SELECT SUM(positiveBonus)-SUM(negativeBonus) AS sumAllBonus FROM CRM.dbo.crm_adminUpDownBonus  WHERE adminId=$adminId and isUsed=0 AND CONVERT(DATE,TimeStamp)>='$EMPTYDATE'");
        if($historyExist){
            $all_monthly_bonuses=$historyExist[0]->sumAllBonus;
        }
        $all_bonus_since_Empty+=$all_monthly_bonuses;
